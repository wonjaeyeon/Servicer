...
class OrderInfoDataGridSource extends DataGridSource {
    ...
    dataGridRows = isWebOrDesktop
        ? orders.map<DataGridRow>((OrderInfo order) {
            return DataGridRow(cells: <DataGridCell>[
              ...
              DataGridCell<String>(
                  columnName: getColumnName('shoppingMall'),
                  value: order.shoppingMall),
              DataGridCell<String>(
    ...
    if (isWebOrDesktop) {
      return DataGridRowAdapter(color: backgroundColor, cells: <Widget>[
        ...
        Container(
          ...
          // ),
          child: getshoppingMallWidget(row.getCells()[2].value.toString()),
        ),
        Container(
          padding: const EdgeInsets.all(8),
          alignment: Alignment.centerLeft,
  ...
  //  Order Data's
  final List<String> _mallNames = <String>[
    '스마트스토어',
    '지그재그',
    '11번가',
    '에이블리',
  ...
  List<OrderInfo> getOrders(List<OrderInfo> orderData, int count,
    ...
    for (int i = startIndex; i < endIndex; i++) {
      orderData.add(OrderInfo(
        ...
        1700 + i,
        _mallNames[
            i < _mallNames.length ? i : _random.nextInt(_mallNames.length - 1)],
...
class OrderInfo {
  ...
  OrderInfo(this.id, this.customerId, this.shoppingMall, this.name,
      this.productName, this.productPrice, this.order, this.totalEarnings);
