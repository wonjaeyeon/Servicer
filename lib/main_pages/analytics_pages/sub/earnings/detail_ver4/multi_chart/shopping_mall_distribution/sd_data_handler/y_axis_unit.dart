...
mixin TDChartYAxisUnit {
  ...
  String getAbsValueUnit(double value) {
    String result = '';
    if (value >= 100000000) {
      if (value % 100000000 == 0) {
        result = '${(value / 100000000).abs().toStringAsFixed(0)}억';
      } else {
        String subResult = getAbsValueUnit((value % 100000000).abs());
        result =
            '${(value / 100000000).abs().toStringAsFixed(0)}억${subResult.substring(0, subResult.length - 1)}';
      }
    } else if (value >= 10000) {
      //check string ${(value / 10000).toStringAsFixed(1)} end with 0
      //if it ends with 0, remove 0
      String valueString = '${(value / 10000).abs().toStringAsFixed(1)}';
      if (valueString.endsWith('0')) {
        result = addComma('${(value / 10000).abs().toStringAsFixed(0)}') + '만';
      } else {
        result = addCommaWithoutDecimal(
                '${(value / 10000).abs().toStringAsFixed(1)}') +
            '만';
      }
    } else {
      result = addComma('${(value).abs().toStringAsFixed(0)}');
    }
    return result + "원";
  }
