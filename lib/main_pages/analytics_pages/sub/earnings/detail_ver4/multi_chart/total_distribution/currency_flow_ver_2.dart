...
class CurrencyFlowChartV2 extends StatefulWidget {
  final double width;
  final Time time;
  const CurrencyFlowChartV2({Key? key, required this.time, required this.width})
      : super(key: key);

  @override
  CurrencyFlowChartV2State createState() => CurrencyFlowChartV2State();
}

class CurrencyFlowChartV2State extends State<CurrencyFlowChartV2> {
  ...
  void buildDataGridRows() {
    dataGridRows = orders.map<DataGridRow>((OrderInfo order) {
      return DataGridRow(cells: <DataGridCell>[
        ...
        DataGridCell<int>(
            columnName: getColumnName('customerId'), value: order.customerId),
        DataGridCell<String>(
            columnName: getColumnName('gender'), value: order.gender),
        DataGridCell<String>(
            columnName: getColumnName('name'), value: order.name),
        DataGridCell<String>(
            columnName: getColumnName('productName'), value: order.productName),
        DataGridCell<double>(
        ...
        DataGridCell<double>(
            columnName: getColumnName('price'), value: order.totalEarnings),
      ]);
    }).toList();
  }
