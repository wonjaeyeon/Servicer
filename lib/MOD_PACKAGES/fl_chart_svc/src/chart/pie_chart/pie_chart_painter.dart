...
import '../../../src/chart/base/line.dart';
...
class PieChartPainter extends BaseChartPainter<PieChartData> {
  ...
  @visibleForTesting
  void drawCenterSpace(CanvasWrapper canvasWrapper, double centerRadius, PaintHolder<PieChartData> holder) {
    final data = holder.data;
  ...
  Path generateSectionPath(
    ...
    if (sectionSpace != 0) {
      final startLineSeparatorPath = createRectPathAroundLine(Line(startLineFrom, startLineTo), sectionSpace);
  ...
  @visibleForTesting
  void drawSection(PieChartSectionData section, Path sectionPath, CanvasWrapper canvasWrapper) {
    _sectionPaint.color = section.color;
  ...
  @visibleForTesting
  void drawSectionStroke(PieChartSectionData section, Path sectionPath, CanvasWrapper canvasWrapper, Size viewSize) {
    ...
    for (var i = 0; i < data.sections.length; i++) {
      ...
      if (section.showTitle) {
        ...
        final tp = TextPainter(
            text: span, textAlign: TextAlign.center, textDirection: TextDirection.ltr, textScaleFactor: holder.textScale);
